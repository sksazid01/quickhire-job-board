...
function adminAuth(request, response, next) {
  const key = (request.headers["x-admin-key"] || "").trim();

  if (!key || key !== config.adminSecret) {
    return response.status(401).json({ message: "Unauthorized. Valid X-Admin-Key header required." });
  }
...
app.post("/api/jobs", adminAuth, async (request, response) => {
...
app.delete("/api/jobs/:id", adminAuth, async (request, response) => {
  const id = Number(request.params.id);
